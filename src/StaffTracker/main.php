    ...
    public function onCommandExecute(PlayerCommandPreprocessEvent $event) {
		...
		$time = date("m-d-Y H:i:s", $time1);
			...
			if ($event->getPlayer()->isOp()) {
				if ($message == "/kick" or $message == "/ban" or $message == "/ban-ip" or $message == "/pardon" or $message == "/pardon-ip" or $message == "/stop" or $message == "/op" or $message == "/deop" or $message == "/give") {
				...
				if ($message == "/kick" or $message == "/ban" or $message == "/ban-ip" or $message == "/pardon" or $message == "/pardon-ip" or $message == "/stop" or $message == "/op" or $message == "/deop" or $message == "/give") {
						$this->log->set($time, "The player: " . $player . " | The command: " . $command);
						$this->log->save();
						$this->log->getAll();
				}
			}
